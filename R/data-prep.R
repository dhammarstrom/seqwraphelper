...
#'
#' Currently, a data sets from Pillon et al. is made
#' available for the user
#' after downloading from the Gene Expression Omnibus. The dataset can be used
#' for replicating analyses in seqwrap vignette and paper.
#'
...
geo_data <- function(dir = "./data-raw", gse = c("GSE202295")) {
  ...
  if (gse == "GSE202295") {
    ...
    count_file <- system.file(
      "extdata",
      "GSE202295_raw_counts_GRCh38.p13_NCBI.tsv",
    ...
    if (count_file == "" || annotation_file == "") {
      stop(
        "Data files not found in package. Please check file names and package installation."
      )
    }
    ...
    GSE202295_countdata <- dat |>
      dplyr::inner_join(dplyr::select(annotation, GeneID, Symbol)) |>
      dplyr::select(geneid = Symbol, tidyselect::starts_with("GSM"))

    # Use system.file() for the series matrix file as well
    series_matrix_file <- system.file(
      "extdata",
      "GSE202295_series_matrix.txt",
      package = "seqwraphelper"
    )
    ...
    meta_data <- data.frame(
      seq_sample_id = gse$geo_accession,
    ...
    out <- list(metadata = meta_data, countdata = GSE202295_countdata)
  }
